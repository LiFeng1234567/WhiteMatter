...
# ============================================================================
# Script: process_bin200_extra.R
# Purpose: Generate a Seurat object (bin200 level) from Stereo-seq extra‑cellular
#          (cell_label == 0) data restricted to corpus callosum regions.
# Usage:   Rscript process_bin200_extra.R <sample_id>
# Example: Rscript process_bin200_extra.R 001
# ============================================================================

# 0. Parse command line argument ---------------------------------------------
args <- commandArgs(TRUE)
if (length(args) < 1) {
  stop("Please provide a sample ID, e.g.: Rscript process_bin200_extra.R 001")
}
sample <- args[1]
...
# Output file
out_rds       <- file.path(base_dir, paste0(sample, "_extra_bin200.rds"))
...
# Keep only EXTRA‑cellular transcripts (cell_label == 0)
extra_data <- data[data$cell_label == 0, ]
...
# Check coverage: all CC loci present in our extra‑cellular data?
cc_loci <- unique(cc_pos$loci)
data_loci <- unique(extra_data$loci)
missing <- setdiff(cc_loci, data_loci)
if (length(missing) > 0) {
  warning("Some CC loci have no extra‑cellular data. Missing: ", paste(missing, collapse = ", "))
}
cat("CC loci coverage: ", length(intersect(cc_loci, data_loci)), " / ", length(cc_loci), "\n")

# Subset extra‑cellular data to CC loci only
extra_cc <- extra_data %>% filter(loci %in% cc_loci)
